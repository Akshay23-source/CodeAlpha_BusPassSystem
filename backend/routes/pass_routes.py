...
def apply_pass():
    ...
    # Log metrics to MongoDB Atlas for charts analytics
    try:
        from services.mongodb_service import MongoDBService
        mongo_service = MongoDBService()
        mongo_service.log_transaction(user.email, float(amount), "Gateway", "success", f"TXN-2026-{new_pass.id}")
        mongo_service.log_boarding(user.email, route, pass_type)
    except Exception as e:
        print(f"Failed to push analytics to MongoDB Atlas: {e}")
